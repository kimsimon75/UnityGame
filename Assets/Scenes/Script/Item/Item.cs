...
public class List
{
    ...
    public List(PlayerStats stats, CannonManager cannon, ItemManager itemManager)
    {
        ...
        50, 0, 0, 0, 0, 0, 0f, 0f, 0, 0, 0, 0// 공격력, 방어력 감소, 마법증폭, 마법방어력 감소, 트루 데미지, 체젠, 마젠, 이동속도 감소, 공격속도, 타워 데미지, 타워 공속
        , 0, 0, 0, 0, 0, 0f, 0f, 5f//확률, 단일 물리 데미지, 범위 물리 데미지, 단일 마법데미지, 범위 마법 데미지, 단일 스턴, 범위 스턴, 스킬 범위
        ,0, 0, 0, 0, 0},//단일 퍼센트 데미지, 끝딜 퍼센트 데미지, 전퍼, 현퍼, 잃퍼
        ...
        1000, 0, 0, 0, 0, 20, 0.3f, 0.3f, 0, 0, 10, 0 // 공격력, 방어력 감소, 마법증폭, 마법방어력 감소, 트루 데미지, 체젠, 마젠, 이동속도 감소, 공격속도, 타워 데미지, 타워 공속
        , 0, 0, 0, 0, 0, 0f, 0f,5f//확률, 단일 물리 데미지, 범위 물리 데미지, 단일 마법데미지, 범위 마법 데미지, 단일 스턴, 범위 스턴, 스킬 범위
        ,0, 0, 0, 0, 0},//단일 퍼센트 데미지, 끝딜 퍼센트 데미지, 전퍼, 현퍼, 잃퍼
    ...
    public Dictionary<string, int> CombineAllItem(Item item, bool combine)
    {
        ...
        while (!isOkay)
        {
            ...
            foreach (KeyValuePair<string, int> kvp in itemDict.ToList())
            {
                ...
                if (kvp.Key == "만물석") continue;
                if (dict[kvp.Key].count < kvp.Value)
                {
                    isOkay = false;
                    string Key = kvp.Key;
                    int necessaryCount = Mathf.Max(kvp.Value - dict[Key].count, 0);
                    foreach (ItemIngredient nItem in dict[Key].NecessaryItem)
                    {
                        if (itemDict.ContainsKey(nItem.Item.Name))
                        {
                            itemDict[nItem.Item.Name] += necessaryCount * nItem.Count;
                        }
                        else
                            itemDict.Add(nItem.Item.Name, necessaryCount * nItem.Count);
                    }
                    itemDict[Key] -= necessaryCount;
                    if (itemDict[Key] <= 0)
                        itemDict.Remove(Key);
                }
            }
    ...
    public Dictionary<string, int> DissolutionAll(Item item)
    {
        Dictionary<string, int> itemDict = new Dictionary<string, int>();
        if (item.Name == "만물석" || item.NecessaryItem[0].Item.Name == "만물석") return itemDict;
        bool isOkay = false;
        foreach (ItemIngredient nItem in item.NecessaryItem)
        {
            itemDict.Add(nItem.Item.Name, nItem.Count);
        ...
        while (!isOkay)
        {
            ...
            foreach (KeyValuePair<string, int> kvp in itemDict.ToList())
            {
                ...
                if (dict[kvp.Key].NecessaryItem[0].Item.Name == "만물석") continue;
                if (dict[kvp.Key].count < kvp.Value)
                {
                    isOkay = false;
                    string Key = kvp.Key;
